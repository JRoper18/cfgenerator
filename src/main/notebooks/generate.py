...
dataset_strs = []
with open('../../../output/output-%s.txt' % eval_run_name, 'r') as data:
...
eval_loader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=False, num_workers=0)
with open('../../../output/gpt-eval-%s,txt' % eval_run_name, 'a') as file:
